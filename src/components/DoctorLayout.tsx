...
import { Stethoscope, User, ClipboardList, Pill, Menu, LogOut } from "lucide-react";
...
export function DoctorLayout() {
  ...
  const navItems = [
    { path: "/doctor", icon: Stethoscope, label: "Today's Sessions" },
    { path: "/doctor/patients", icon: User, label: "Patients" },
    { path: "/doctor/procedures", icon: ClipboardList, label: "Procedures" },
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-[#1B2A4A]/80 border-b border-white/10 flex items-center justify-between px-4 md:px-6">
          ...
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-lg font-semibold text-white">Doctor Dashboard</h1>
            <p className="text-xs text-white/50">لوحة الطبيب</p>
          </div>
