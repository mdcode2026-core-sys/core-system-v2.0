...
export default function AuthScreen({ onSuccess }: AuthScreenProps) {
  ...
  const getKeyIcon = (key: string) => {
    if (key === "backspace") return <span className="text-lg md:text-xl font-bold">⌫</span>;
    if (key === "clear") return <span className="text-xs md:text-sm font-bold">CLR</span>;
    return <span className="text-2xl md:text-3xl font-bold">{key}</span>;
  };
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 gap-2 bg-white/10 rounded-lg p-1 mb-6">
              <button
                ...
                className={`py-2 px-4 rounded-md text-sm md:text-base font-medium transition-all ${activeTab === "email" ? "bg-white text-[#1B2A4A]" : "text-white/70 hover:text-white"}`}
              >
              ...
              <button
                ...
                className={`py-2 px-4 rounded-md text-sm md:text-base font-medium transition-all ${activeTab === "pin" ? "bg-white text-[#1B2A4A]" : "text-white/70 hover:text-white"}`}
              >
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full h-12 md:h-14 text-base md:text-lg bg-white/5 border border-white/20 text-white placeholder:text-white/30 rounded-lg px-4 focus:outline-none focus:border-white/40"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-white/70 flex items-center gap-2">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full h-12 md:h-14 text-base md:text-lg bg-white/5 border border-white/20 text-white placeholder:text-white/30 rounded-lg px-4 pr-12 focus:outline-none focus:border-white/40"
                    />
                    <button
                      type="button"
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-center gap-3 mb-4">
                  {[0, 1, 2, 3].map((index) => (
                    <div
                      key={index}
                      className={`w-12 h-12 md:w-16 md:h-16 rounded-xl flex items-center justify-center transition-all duration-200 ${
                        index === pinIndex
                    ...
                    >
                      {pin[index] ? <div className="w-3 h-3 md:w-4 md:h-4 rounded-full bg-white" /> : <span className="text-white/20 text-lg">{index + 1}</span>}
                    </div>
                  ))}
                </div>
